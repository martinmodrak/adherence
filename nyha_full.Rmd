...
```{r}
data_groups %>% group_by(adherence_category, diagnosis) %>% summarise(count = n(), .groups = "drop_last") %>%
  mutate(percent = scales::percent(count / sum(count)), count_percent = paste0(count, " (", percent, ")")) %>%
  arrange(adherence_category, desc(count), diagnosis) %>%
  select(adherence_category, diagnosis, count_percent) %>%
  table_format(caption = "Diagnosis by adherece category")
```

...
data_long |> filter(drug_class == "BB") |> 
 mutate(adherent = fct_recode(adherent, "N/M" = "cannot_measure", "N/T" = "not_using"),
  ...
  filter(!is.na(ekg_freq)) |> 
  group_by(cohort, adherent) |> 
  summarise(EKG_median = median(ekg_freq), EKG_IQR = IQR(ekg_freq), EKG_min = min(ekg_freq), EKG_max = max(ekg_freq))

data_BB_test <- data_long |> filter(drug_class == "BB", !is.na(ekg_freq), adherent %in% c("yes","no"))
t_2018 <- t.test(log(ekg_freq) ~ adherent, data_BB_test |> filter(cohort == "2018"))
t_2018
exp(t_2018$conf.int)
t_2020 <- t.test(log(ekg_freq) ~ adherent, data_BB_test |> filter(cohort == "2020"))
t_2020
exp(t_2020$conf.int)
...
adherent_diagnosis_contingency <- data_wide_a %>% group_by(category_bin, diagnosis_group) %>%
  ...
  tibble::column_to_rownames("category_bin") %>%
  as.matrix()

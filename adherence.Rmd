...
```{r}
both_cohorts_drugs <- data_long %>% filter(!is.na(adherent_int)) %>% select(drug_class, cohort) %>% distinct() %>%
  group_by(drug_class) %>% summarise(both_cohorts = length(unique(cohort)) == 2) %>% filter(both_cohorts) %>% pull(drug_class)
```


For the main focus on adherence categories we'll limit the look to drugs measured in both cohorts, i.e. `r paste0(both_cohorts_drugs, collapse = ", ")`.
...
The differences float around the standard significant threshold with Chi-squared p-values of `r round(chisq_adherent_cat_by_cohort$p.value, 2)` and `r round(chisq_adherent_cat_bin_by_cohort$p.value, 2)` for the fine categories and for the binary categorization respectively.
...
data_table_comparison <- data_table1 %>%
  ...
  select(-N_adherent, -N_drugs, -category, -N_nonadherent)

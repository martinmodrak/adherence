...
data_table1 <- data_wide %>% 
  ...
  select(
    subject_id,
  sex, age, weight, height, BMI, ekg_freq, pulse, systolic_BP, diastolic_BP, NYHA, EF_numeric, starts_with("lab_"), starts_with("has."), cohort
) 
...
Summary by cohorts. P-values for tests are: Wilcoxon test for continous, Chi-squared test for all categorical.
...
Comparison of clinical and laboratory parameters of adherent and non-adherent patients (excluding furosemide):

```{r, message=FALSE}
data_table_comparison <- data_table1 %>%
  inner_join(data_long_sum_no_fur, by = c("subject_id", "cohort"), relationship = "one-to-one", unmatched = "error")

tbl_summary(data_table_comparison, by = category_bin, statistic = tbl1_stats)  %>% add_p(test = tbl1_test)
```
...
```{r}
data_wide_sum <- data_wide %>% inner_join(data_long_sum, by = c("subject_id", "cohort"),
...
if(nrow(data_wide_a) != nrow(data_wide)) {
  stop("Bad join")
}
```

```{r}
adherent_diagnosis_contingency <- data_wide_a %>% group_by(adherence_category, diagnosis_group) %>%
  tally() %>%
  pivot_wider(names_from = "diagnosis_group", values_from = "n") %>%
  column_to_rownames("adherence_category") %>%
  as.matrix()
